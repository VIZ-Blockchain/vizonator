...
`;
	...
	function vizonator_button_click(e){
		...
		let beneficiaries=decodeURIComponent($(this).attr('data-beneficiaries'));
	...
	function continuously_view(){
		$('.page.preview').each(function(i,el){
			if(!$(el).hasClass('activated_vizonator')){
				$(el).addClass('activated_vizonator');
				let award_data=' data-login="'+$(el).attr('data-content-author')+'" data-sequence="0" data-memo="'+$(el).attr('data-content-author')+'/'+$(el).attr('data-content-permlink')+'"';
				if(typeof $(el).attr('data-beneficiaries') !== 'undefined'){
					award_data+=' data-beneficiaries="'+encodeURIComponent($(el).attr('data-beneficiaries'))+'"';
				}
				else{
					award_data+=' data-beneficiaries="[]"';
				}
				$(el).append('<div class="vizonator_div"><div class="vizonator_button"'+award_data+' data-id="'+vizonator_counter+'">'+vizonator_icon+' Award</div><div class="vizonator_status"></div></div>');
		...
		$('.page.content').each(function(i,el){
			if(typeof $(el).attr('data-content-author') !== 'undefined'){
				if(!$(el).hasClass('activated_vizonator')){
					$(el).addClass('activated_vizonator');
					let award_data=' data-login="'+$(el).attr('data-content-author')+'" data-sequence="0" data-memo="'+$(el).attr('data-content-author')+'/'+$(el).attr('data-content-permlink')+'"';
					if(typeof $(el).attr('data-beneficiaries') !== 'undefined'){
		...
		setTimeout(function(){continuously_view()},2000);
	}
	function init_page(){
		wait_loading('.page','','',function(){
			continuously_view();
		});
	}
	$(function(){
