...
`;
	...
	function vizonator_button_click(e){
		...
		let user_name=$(this).closest('.td_headerandpost').prev().find('a:first-child').attr('href');
		if(-1!=user_name.indexOf('u=')){
			user_name=user_name.substr(user_name.indexOf('u=')+2);
		}
		console.log(user_name);
		let item_link=$(this).parent().next().attr('href');
		console.log(item_link);
		if(typeof item_link === 'undefined'){
			item_link=window.location.pathname;
		}
		let item_id=item_link.split('topic=')[1];
		if(-1!=item_id.indexOf('#')){
			item_id=item_id.substr(0,item_id.indexOf('#'));
		}
		if(-1!=item_id.indexOf('.msg')){
			item_id=item_id.replace('.msg',' ');
		}
		if(-1!=item_id.indexOf('.new')){
			item_id=item_id.replace('.new','');
		}
		console.log(item_id);
	...
	function continuously_view(){
		$('td.td_headerandpost table td.td_buttons').each(function(i,el){
			if(!$(el).hasClass('activated_vizonator')){
				$(el).addClass('activated_vizonator');
				$(el).find('a:last-child').before('<div class="vizonator_div"><div class="vizonator_button page" data-id="'+vizonator_counter+'">'+vizonator_icon+' Award</div><div class="vizonator_status"></div></div>');
	...
	function init_page(){
		wait_loading('#bodyarea','','',function(){
			continuously_view();
		});
	}
	$(function(){
