...
.vizonator-dark .vizonator_status{
	color:#8899a6;
}
.vizonator-dark .vizonator_button{
	color:#ddd;
}
</style>
`;
	...
	function check_dark(){
		$('html').removeClass('vizonator-dark');
		let css_str=$('body').css('background-color');
		css_str=css_str.replaceAll(' ','');
		console.log(css_str);
		let dark=true;
		if('#fff'==css_str){
			dark=false;
		...
		if('rgb(255,255,255)'==css_str){
			dark=false;
		}
		if(dark){
			$('html').addClass('vizonator-dark');
		}
	}
	...
	function continuously_view(){
		$('article[role=article]').each(function(i,el){
			if(!$(el).parent().parent().hasClass('activated_vizonator')){
				$(el).parent().parent().addClass('activated_vizonator');
				$(el).find('div[role=group]:last-child').each(function(i2,el2){
					if(!$(el2).hasClass('activated_vizonator')){
						$(el2).addClass('activated_vizonator');
						$(el2).after('<div class="vizonator_div"><div class="vizonator_button" data-id="'+vizonator_counter+'">'+vizonator_icon+' Award</div><div class="vizonator_status"></div></div>');
	...
	function init_page(){
		wait_loading('article[role=article]','','',function(){
			continuously_view();
			check_dark();
		});
	}
