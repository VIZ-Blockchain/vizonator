...
`;
	...
	function vizonator_button_click(e){
		...
		let item_link=$(this).closest('.sound__body').find('a.soundTitle__title').attr('href');
		console.log(item_link);
		if(typeof item_link === 'undefined'){
			item_link=window.location.pathname;
		}
		let user_name=item_link.split('/')[1];
		let item_id=item_link.split('/')[2];
		if('sets'==item_id){
			item_id=item_link.split('/')[2]+'/'+item_link.split('/')[3];
		}
		let memo=user_name+' '+item_id;
	...
	function continuously_view(){
		$('.listenEngagement__footer').each(function(i,el){
			if(!$(el).hasClass('activated_vizonator')){
				$(el).addClass('activated_vizonator');
				$(el).after('<div class="vizonator_div"><div class="vizonator_button page" data-id="'+vizonator_counter+'">'+vizonator_icon+' Award</div><div class="vizonator_status"></div></div>');
		...
		$('.sound__footer').each(function(i,el){
			if(!$(el).hasClass('activated_vizonator')){
				$(el).addClass('activated_vizonator');
				$(el).after('<div class="vizonator_div"><div class="vizonator_button page" data-id="'+vizonator_counter+'">'+vizonator_icon+' Award</div><div class="vizonator_status"></div></div>');
				$('.vizonator_button[data-id="'+vizonator_counter+'"]').off('click');
		...
		/*
		$('.sound.streamContext').each(function(i,el){
			if(!$(el).hasClass('activated_vizonator')){
				$(el).addClass('activated_vizonator');
				$(el).find('.sound__footer').after('<div class="vizonator_div"><div class="vizonator_button" data-id="'+vizonator_counter+'">'+vizonator_icon+' Award</div><div class="vizonator_status"></div></div>');
				$('.vizonator_button[data-id='+vizonator_counter+']').unbind('click');
				$('.vizonator_button[data-id='+vizonator_counter+']').bind('click',vizonator_button_click);
				vizonator_counter++;
			}
		});
		*/
