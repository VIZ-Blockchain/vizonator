...
`;
	...
	function vizonator_button_click(e){
		...
		let login='social';
		let sequence=3;
		let user_name=$(this).closest('article').find('a:first').attr('href');
		user_name=user_name.split('/')[1];
		let item_id=$(this).closest('article').find('time').parent().attr('href');
	...
	function continuously_view(){
		$('article').each(function(i,el){
			if(!$(el).hasClass('activated_vizonator')){
				...
				$(el).find('section:first').after('<div class="vizonator_div"><div class="vizonator_button" data-id="'+vizonator_counter+'">'+vizonator_icon+'</div><div class="vizonator_status"></div></div>');
